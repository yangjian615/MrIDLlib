...
;
; PURPOSE:
;	MR_FORM is a compound widget that simplifies creating
...
;		and its value returned as the result of MR_FORM.  For non-
...
;  *****************
;
;    To use MR_FORM inside another widget:
;	    a = widget_base(title='Testing')
;	    b = mr_form(a, desc, /COLUMN)
...
;   11 May 2012     MR_FORM_PARSE returns 0 or 1 if EVENT is specified
;                   in the DESC string. As a result, the check of N_ELEMENTS(enf)
;                   passes if a previous EVENT function had been specified, but no
;                   EVENT function was specified for the current widget. Changed
;                   this behavior by adding "&& enf" - MRA
;-
;


function MR_FORM_PARSE, Extra, Name, Value, Index=Index
...
pro MR_FORM_APPEND, extra, e, keyword, USE_VALUE=use_value, ACTUAL_KEYWORD=akw
  COMPILE_OPT hidden

if MR_FORM_PARSE(e, keyword, value) then begin
...
pro MR_FORM_LABEL, parent, nparent, e, frame
;Put LABEL_LEFT and/or LABEL_RIGHT on a base.

  COMPILE_OPT hidden

nparent = parent
if MR_FORM_PARSE(e, 'LABEL_LEFT', value) then begin
	nparent = WIDGET_BASE(nparent, /ROW, FRAME=frame)
	frame = 0
	junk1 = WIDGET_LABEL(nparent, VALUE=value)
	endif
if MR_FORM_PARSE(e, 'LABEL_TOP', value) then begin
...
pro MR_FORM_BUILD, parent, desc, cur, ids, lasttag
...
;
  ...
  while cur lt n do begin
	...
	quit = MR_FORM_PARSE(e, 'QUIT')
	frame = MR_FORM_PARSE(e, 'FRAME')
	efn = MR_FORM_PARSE(e, 'EVENT', event_fun)
	MR_FORM_APPEND, extra, e, 'FONT', /USE_VALUE
	MR_FORM_APPEND, extra, e, 'COLUMN'
	MR_FORM_APPEND, extra, e, 'ROW'
	MR_FORM_APPEND, extra, e, 'LEFT', ACTUAL='ALIGN_LEFT'
	MR_FORM_APPEND, extra, e, 'CENTER', ACTUAL='ALIGN_CENTER'
	MR_FORM_APPEND, extra, e, 'RIGHT', ACTUAL='ALIGN_RIGHT'
    endif else e = ''
...
'BASE': BEGIN
    ...
    ENDCASE
'BUTTON': BEGIN
    MR_FORM_APPEND, extra, e, 'LABEL_LEFT', /USE_VALUE
    MR_FORM_APPEND, extra, e, 'LABEL_TOP', /USE_VALUE
    exclusive = MR_FORM_PARSE(e,'EXCLUSIVE')
    no_release = MR_FORM_PARSE(e,'NO_RELEASE')
    values = strtok(a[2],'|', /EXTRACT, ESC='\')
    if n_elements(values) ge 2 then begin
        type = 0
        if MR_FORM_PARSE(e, 'SET_VALUE', temp) then begin
...
process_integer:
    uextra = { VALUE: value }
    MR_FORM_LABEL, parent, nparent, e, frame
    if MR_FORM_PARSE(e, 'WIDTH', temp) then width = fix(temp) else width=6
...
'LIST': BEGIN
    ...
    if MR_FORM_PARSE(e, 'HEIGHT', temp) eq 0 then temp = n_elements(v)
...
ENDCASE
	...
	if MR_FORM_PARSE(e, 'TAG', value) then value = STRUPCASE(value) $
	...
	lasttag = new
	;MRA 05/11/2012 -- && efn
	if (N_ELEMENTS(efn) NE 0) && efn AND (N_ELEMENTS(event_fun) NE 0) then $
    ...
    if dflags and 1 then MR_FORM_BUILD, new, desc, cur, ids, lasttag  ;Begin new
...
Function MR_FORM_EVENT, ev		;Event handler for MR_FORM
  COMPILE_OPT hidden
...
Pro MR_FORM_SETV, id, value	;In this case, value = { Tagname : value, ... }
  COMPILE_OPT hidden
...
Function MR_FORM_GETV, id	;Return value of a MR_FORM widget.
  COMPILE_OPT hidden
...
;  ON_ERROR, 2						;return to caller
  ...
  IF (NOT KEYWORD_SET(uname)) THEN UNAME='MR_FORM_UNAME'
  ...
  MR_FORM_BUILD, base, desc, 0, ids, lasttag
  widget_control, base, EVENT_FUNC='MR_FORM_EVENT', $
	FUNC_GET_VALUE='MR_FORM_GETV', PRO_SET_VALUE='MR_FORM_SETV'

  if n_params() eq 1 then begin		;Modal?
	WIDGET_CONTROL, p, /realize
	XMANAGER, 'MR_FORM', p, EVENT_HANDLER='MR_FORM_MODAL_EVENT'
        v = TEMPORARY(*handle)
	PTR_FREE, handle
