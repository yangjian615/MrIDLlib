...
;
; PURPOSE:
;   MR_BGROUP is a compound widget that simplifies creating
...
;   handled transparently, and a MR_BGROUP event returned. This
...
;
; CALLING SEQUENCE:
;       Widget = MR_BGROUP(Parent, Names)
;
;   To get or set the value of a MR_BGROUP, use the GET_VALUE and
;   SET_VALUE keywords to WIDGET_CONTROL. The value of a MR_BGROUP
;   is:
;
;       -----------------------------------------------
;       Type        Value
...
;   11 April 2012, MRA  Added SENSITIVE keyword to set initial
;                       sensitivity state. Made a BUTTON_UNAME
;                       field. The UVALUE field of each button
;                       is used as a placeholder for accessing
;                       arrays. BUTTON_UVALUE is only returned
;                       after a button event. BUTTON_UNAME can
;                       can be accessed by WIDGET_CONTROL.
;-
pro MR_BGROUP_SETV, id, value
  compile_opt hidden
...
function MR_BGROUP_GETV, id, value
...
function MR_BGROUP, parent, names, $
    BUTTON_UVALUE = button_uvalue, BUTTON_UNAME = button_uname, COLUMN=column, $
    EVENT_FUNCT = efun, EXCLUSIVE=excl, FONT=font, FRAME=frame, IDS=ids, $
    LABEL_TOP=label_top, LABEL_LEFT=label_left, MAP=map, $
    NONEXCLUSIVE=nonexcl, NO_RELEASE=no_release, RETURN_ID=return_id, $
    RETURN_INDEX=return_index, RETURN_NAME=return_name, $
    ROW=row, SENSITIVE=sensitive, SCROLL=scroll, SET_VALUE=sval, SPACE=space, $
  ...
  IF (N_ELEMENTS(uname) eq 0)      then uname = 'MR_BGROUP_UNAME'
  ...
  ;------------------------------------------;
  ;add sensitivity state. default to sensitive
  n = n_elements(names)
  nsens = n_elements(sensitive)
  if nsens eq 0 then sensitive = replicate(1, n) else $
  if nsens eq 1 then sensitive = replicate(sensitive, n) else $
  if nsens ne n then print, 'SENSITIVE has wrong number of elements.'

  ;------------------------------------------;
  ;if BUTTON_UVALUE is not given, make it equal to the index number of the button
  ;this way, the uvalue can be accessed even without a button event.
  if n_elements(button_uname) eq 0 then begin
    button_uname = names + '_BUTTON' + strtrim(indgen(n), 2) 
  endif else if n_elements(button_uname) ne n then $
    print, 'BUTTON_UNAME must have same # elements as NAMES'
  ...
  WIDGET_CONTROL, top_base, MAP=map, $
    FUNC_GET_VALUE='MR_BGROUP_GETV', PRO_SET_VALUE='MR_BGROUP_SETV', $
  ...
  for i = 0, n-1 do begin
    if (n_elements(font) eq 0) then begin
      ...
      UNAME=button_uname[i], SENSITIVE=sensitive[i]) ;UNAME=uname+'_BUTTON'+STRTRIM(i,2)
    endif else begin
  ...
  if (n_elements(sval) ne 0) then MR_BGROUP_SETV, top_base, sval
